...
export function getRehostingInfo (rehoster) {
  const nrReplicated = rehoster.replicatedDiscoveryKeys.length
  const nrServed = rehoster.servedDiscoveryKeys.length

  let res = '\nRehoster\n'
  res += ` - public key: ${asHex(rehoster.ownKey)}\n`
  res += ` - rehosted cores: ${nrReplicated} (announced: ${nrServed}) \n`

  res += 'Swarm\n'
  const swarmInfo = getSwarmInfo(rehoster.swarmInterface.swarm)
...
export async function getRehostingDetails (rehoster) {
  ...
  for (const n of nodes) {
    infos.push(
      ` - ${n.pubKey} -- ${n.announcing ? 'announcing' : '/'} -- ${n.downloadedBlocks} blocks -- ${n.nrPeers} peers`
    )
  }
