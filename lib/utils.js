...
export function getNrConnections (swarm) {
...
function getSwarmInfo (swarm) {
  ...
  return {
    ...
    'nr connections': getNrConnections(swarm)
  }
}
...
export async function collectRawNodes (rehoster) {
  const nodes = []
  const graph = asDiGraph(rehoster)
  for await (const node of graph.yieldAllNodesOnce()) {
    nodes.push(node.rehosterNode)
  }

  return nodes
}

export async function collectNodes (rehoster) {
  const nodes = await collectRawNodes(rehoster)
  return nodes.map(n => getNodeInfo(n))
}
...
export async function getRehostingDetails (rehoster) {
  ...
  for (const n of nodes) {
    let res = ` - ${n.pubKey} -- ${n.announcing ? 'announcing' : '/'} -- ${byteSize(n.nrBytes)} -- ${n.downloadedBlocks} blocks -- ${n.nrPeers} peers`
    if (n.info) res += `\n     - Info: '${n.info}'`
